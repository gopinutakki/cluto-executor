			...
			for(int cm = 0; cm < clusteringMethods.size(); cm++)
				for(int sm = 0; sm < similarityMeasures.size(); sm++)
					for(int cf = 0; cf < criterionFunctions.size(); cf++){
						cmd = "vcluster -clmethod=" + clusteringMethods.get(cm)+ " -rclassfile=" + datasets.get(d) + ".rclass -clabelfile=" + datasets.get(d) + ".mat.clabel -nfeatures=8 -showsummaries=cliques -showfeatures -showtree -labeltree -sim=" + similarityMeasures.get(sm)+ " " + datasets.get(d) + ".mat 10" ;
